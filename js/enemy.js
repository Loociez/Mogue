...
function findPath(startX, startY, endX, endY, map) {
  const openSet = [{ x: startX, y: startY, g: 0, h: Math.abs(endX - startX) + Math.abs(endY - startY), f: 0, parent: null }];
...
export class Enemy {
  constructor(x, y, spriteIndex = 0, type = "normal") {
    ...
    // Each enemy animation uses 12 consecutive frames (after skipping 8)
    this.spriteFrameMap = {
      up: [0, 1, 2],
      down: [3, 4, 5],
      left: [6, 7, 8],
  ...
  update(player, frameCount) {
    ...
    if (this.pathUpdateTicker >= 10 || !this.path || this.path.length === 0) {
      this.path = findPath(this.x, this.y, player.x, player.y, map);
      this.pathUpdateTicker = 0;
    }
    ...
    if (this.path && this.path.length > 1) {
      ...
      // Update tile position once reached
  ...
  // --- FIX: give each enemy its own 12-frame block ---
